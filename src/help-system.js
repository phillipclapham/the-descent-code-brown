...
export class HelpSystem {
    ...
    getMaxScroll() {
        ...
        switch (this.currentTab) {
            case 0: lines = this.content.controls; break;
            case 1: lines = this.content.mechanics; break;
            case 2: lines = this.content.strategy; break;
            case 3: lines = this.content.credits; break;
        }
    ...
    render() {
        ...
        this.ctx.fillText('THE DESCENT: CODE BROWN - HELP', this.canvas.width / 2, boxY + 30);
        ...
        this.renderTabContent(boxX + 20, boxY + 130, boxWidth - 40, boxHeight - 195);

        // Draw footer with scroll instructions
        this.ctx.fillStyle = '#00cc00';
        this.ctx.font = '14px "Courier New", monospace';
        this.ctx.textAlign = 'center';
        this.ctx.fillText('H/ESC: Close  |  ←/→ or 1-4: Switch tabs  |  W/S or ↑/↓: Scroll',
                         this.canvas.width / 2, boxY + boxHeight - 15);
    }
    ...
    renderTabs(x, y, width) {
        ...
        this.tabs.forEach((tab, index) => {
            ...
            // Tab background
            this.ctx.fillStyle = isActive ? 'rgba(0, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.3)';
            ...
            this.ctx.font = isActive ? 'bold 16px "Courier New", monospace' : '14px "Courier New", monospace';
            this.ctx.textAlign = 'center';
    ...
    renderTabContent(x, y, width, availableHeight) {
        ...
        if (this.scrollOffset < maxScroll) {
            ...
            this.ctx.font = 'bold 16px "Courier New", monospace';
            this.ctx.textAlign = 'center';
            this.ctx.fillText('↓ More below ↓', x + width / 2, y + availableHeight + 20);
        }
    }
    ...
    getControlsContent() {
        return [
            ...
            { text: 'Diagonal movement supported (W+A, W+D, S+A, S+D)', indent: true },
            { text: '', indent: false },
            ...
            { text: 'Slots 5-8: Consumables (select, then ENTER to use)', indent: true },
            { text: '', indent: false },
            ...
            { text: 'C - Clench (freeze desperation 10s, 60s cooldown)', indent: false },
            { text: 'Use before fights, vault looting, or emergencies', indent: true },
            { text: '', indent: false },
            ...
            { text: 'Walk into doors to open (keys consumed automatically)', indent: false },
    ...
    getMechanicsContent() {
        return [
            ...
            { text: 'Climbs at 0.35% per second (full run: 4-6 minutes)', indent: true },
            ...
            { text: 'Creates "clutch" moments - use before fights or vaults!', indent: true },
            { text: '', indent: false },
            ...
            { text: '50-74% (Orange): -25% speed, +40% damage, -15% accuracy', indent: true },
            { text: '75-89% (Red): -40% speed, +60% damage, -25% accuracy', indent: true },
            { text: '         BASH WEAK WALLS (orange walls)', indent: true },
            { text: '90-99% (Flash): -50% speed, +75% damage, -30% accuracy', indent: true },
            ...
            { text: 'Extra consumables spawn inside - strategic rest spots!', indent: true },
            { text: '', indent: false },
            { text: 'SHRINES 🕊️', header: true },
            { text: 'Cyan Ω altars found in shrine rooms (40% spawn rate)', indent: true },
            ...
            { text: 'Discover ChromaCorp\'s dark history and employee humor', indent: true },
            { text: 'Messages change based on floor theme (office/maintenance/sewer)', indent: true },
            { text: 'Can re-read multiple times for different messages', indent: true }
        ];
    }
    ...
    getStrategyContent() {
        return [
            ...
            { text: 'Find Break Rooms for strategic rests (desperation pauses)', indent: true },
            { text: 'Save shrines for emergencies (+30 HP, -30% desperation)', indent: true },
            { text: 'Antacids reduce 25% desperation (use wisely!)', indent: true },
            { text: 'Coffee gives +30% speed BUT increases 15% desperation', indent: true },
            { text: '', indent: false },
            ...
            { text: 'Wrench: High damage with stun chance (20-28 dmg)', indent: true },
            ...
            { text: 'Find keys to unlock vaults (or wait til 90% to force)', indent: true },
            { text: 'Bash weak walls at 75%+ for shortcuts', indent: true },
            { text: 'Break Rooms are rare - use them wisely', indent: true },
            { text: 'Read environmental features (blue *) for lore and humor', indent: true },
            { text: 'Explore thoroughly for items, shrines, and secrets', indent: true },
            { text: '', indent: false },
    ...
    getCreditsContent() {
        return [
            ...
            { text: '', indent: false },
            { text: 'Created by Phill Clapham', indent: false },
            { text: 'with Claude Code (Anthropic)', indent: false },
            { text: '', indent: false },
            { text: '', indent: false },
            ...
            { text: '1.0.0 (Phase 4 public release)', indent: true },
            { text: '', indent: false },
